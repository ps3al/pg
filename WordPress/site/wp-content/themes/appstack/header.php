...
<header class="header-banner-top">
  ...
  <div class="main-navigation <?php if ( is_user_logged_in() && is_admin_bar_showing() ) { echo 'logged-in'; } ?>">
	<div class="logo">
		 <?php $stack_logo  = get_field('logo','option');
          if(!empty($stack_logo)) { ?>
 	       <a href="<?php echo esc_attr(home_url()); ?>"> <img class="a" src="<?php echo esc_attr($stack_logo['url']);?>" alt=""> </a>
 	       <?php }
 	   		 else { ?>
	 ...
	 <?php $stack_btn = esc_attr(get_field('nav_button', 'option'));
	       $stack_form = esc_attr(get_field('form_id','option'));
