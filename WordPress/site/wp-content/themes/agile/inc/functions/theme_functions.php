...
//add_action( 'edit_category', array(&$this, 'appstack_category_transient_flusher' ) );
add_action( 'edit_category', 'appstack_category_transient_flusher' );
//add_action( 'save_post',     array(&$this, 'appstack_category_transient_flusher' ) );
...
//add_filter( 'body_class', array(&$this, 'appstack_body_classes' ) );
add_filter( 'body_class', 'appstack_body_classes');
...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
	 ...
	 */
	function appstack_wp_title( $title, $sep ) {
		if ( is_feed() ) {
			return $title;
		}
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title .= " $sep " . sprintf( esc_html__( 'Page %s', 'appstack' ), max( $paged, $page ) );
		}

		return $title;
	}
//	add_filter( 'wp_title', array(&$this, 'appstack_wp_title', 10, 2  ) );
	add_filter( 'wp_title', 'appstack_wp_title', 10, 2  );
	 ...
	 * @todo Remove this function when WordPress 4.3 is released.
	 */
	function appstack_render_title() {
		?>
		<title><?php wp_title( '|', true, 'right' ); ?></title>
		<?php
	}
//	add_action( 'wp_head', array(&$this, 'appstack_render_title' ) );
	add_action( 'wp_head', 'appstack_render_title' );
endif;
...
add_filter('acf/load_field/name=form_id', 'stack_acf_load_contact_field');
