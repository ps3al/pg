...
#>
	<# if( data.options ){
		var vals = value.split(',');
		for( var n in data.options ){
			#><input type="checkbox" id="kc-radio-{{data.name}}-{{n}}" class="kc-param" name="{{data.name}}" <#
				if( vals.indexOf( n ) > -1 ){ #> checked<# }
			#> value="{{n}}" /> <label class="rbtn" for="kc-radio-{{data.name}}-{{n}}">{{data.options[n]}}</label>
		<# }
...
?>
	<div class="kc-radio-field-wrp">
		<# if( data.options ){
			for( var n in data.options ){
				#><input type="radio" id="kc-radio-{{data.name}}-{{n}}" class="kc-param" name="{{data.name}}" <#
			...
			<div class="kc-radio-image-field-body">
				<# for( var n in data.options ){
					#><input type="radio" id="kc-radio-{{data.name}}-{{n}}" class="kc-param" name="{{data.name}}" <#
						if( n == data.value ){ #> checked<# }
					#> value="{{n}}" /> 
					<label class="rbtn" for="kc-radio-{{data.name}}-{{n}}"><img src="{{data.options[n]}}" /></label>
				<# } #>
	...
	<#
		data.callback = function( el, $ ){
			...
			el.find('input.kc-param').on('focus', function(){
				...
				var atts = { title: 'Icon Picker (typing to search icons)', width: 600, class: 'no-footer kc-icons-picker-popup', keepCurrentPopups: true };
