		...
		backbone : {
				...
				if( mid !== undefined ){
					...
					if( kc.storage[ mid ] ){
						...
						for( var name in datas ){
							...
							{
								if( map_values[name] !== undefined && typeof( prev[ name ] ) == 'undefined' )
									prev[ name ] = '__empty__';
							}
			...
			settings : function( e, atts ){
				...
				if( atts === undefined || atts.noscroll === undefined || atts.noscroll != 'yes' )
					kc.ui.preventScroll( pop.find('.m-p-body') );
				...
				*/
				...
				popup.add_tab( pop, {
					...
					title: '<i class="et-layers"></i> Presets',
			...
			add	 : function( e ){
				...
				var atts = { title: kc.__.i02, width: 950, class: 'no-footer', float: true };
			...
			presets : function( tab, form ){
				...
				kc.trigger({
					...
					do_add: function(e){
						...
						if( stack[name] !== undefined && !confirm( kc.__.i55 ) ){
							return;
						}
						...
						stack[name] = [ cate, time,kc.tools.base64.encode( full ) ];
			...
			dopush : function( full, el ){
				...
				if( fid !== null ){
					...
					}, 1000, edit.get(0), $('.kc-params-popup.wp-pointer-top') );
			...
			*/
			...
			columnsResize : {
				...
				down : function( e ){
					...
					var ge = kc.ui.columnsResize, el = $( e.target ).parent();
			...
			callbacks : {
				...
				number_slider : function( el, $, data ){
			        ...
			        var el_slider = el.find('.kc_percent_slider'),
						...
						options = {
							...
							ratio : (typeof data.options['ratio'] == 'undefined' )? 1: data.options['ratio'],
							min : (typeof data.options['min'] == 'undefined' )? 1: data.options['min'],
							max : (typeof data.options['max'] == 'undefined' )? 100: data.options['max'],
				...
				autocomplete : function(wrp, $, data) {
					...
					function render( data, wrp ){
						...
						var out = '', post_type = 'any', category = '', category_name = '', numberposts = 120, taxonomy = '';
										...
										if( el.session == result.__session ){
											...
											$(el.parentNode).find('.kc-autp-suggestion ul').html( out.join('') ).find('li').on('click',function(){
												...
												if( value === null || value === undefined )
													return;
												value = value.split(':');
												$(this).closest('.kc_autocomplete_wrp').find('ul.autcp-items').append('<li data-id="'+value[0]+'"><span>'+kc.tools.esc_attr(value[1])+'</span><i class="sl-close kc-ac-remove" title="Remove item"></i></li>');
												helper( $(this).closest('.kc_autocomplete_wrp') );
											});
										}else alert(el.session+' '+result.__session);
									}
								);
						...
						if (f == 'color' && ( values[f] == 'rgba(0, 0, 0, 0)' || values[f] == 'transparent')) {	
							...
							if (typeof values[f] == 'object' && values[f][0] !== undefined) {
								if (values[f][0].indexOf('deg') > -1) {
									wrp.find('.custom-degrees').trigger('click');
									wrp.find('.degrees-row input.degrees').val(values[f][0].replace(/[^0-9\-]/g, ''));
								...
								for (var i=1; i<values[f].length; i++) {
									wrp.find('.add-more-color').trigger('click');
									var inp = wrp.find('.color-row').last().find('input.grdcolor');
									inp.val(values[f][i]);
									inp.get(0).color.importColor();
								}
							}
						} else {
				...
				css : function( wrp, $, data ){
					...
					kc.do_action( 'kc-css-field-change', wrp, pop );
